...
Der Nutzerbereich wird nicht von der `isy-web` zur Verfügung gestellt.
Es gelten die Anforderungen aus dem Styleguide, vor allem hinsichtlich des Verhaltens des Logout-Buttons.

Konkrete Anwendungen müssen über die Konfigurationseinstellung `gui.header.nutzerbereich.xhtml.src` die Datei angeben, welche den Nutzerbereich definiert.
...
* `globalConfig`: Spezifische, globale Konfiguration. (Standard: `globalConfigurationModel`)
* `charpicker`: Gibt an, welche Eingabe für Sonderzeichen verwendet werden soll. Wird der Wert auf `dinSpec` gesetzt, wird der neue Character-Picker verwendet, welcher alle Zeichen der DIN SPEC 91379 unterstützt. Soll der allte Legacy-Character-Picker verwendet werden, muss der Wert auf `special` gesetzt werden. Standardmäßig wird kein Character-Picker verwendet.
...
Für den Fall das kein Parameter gewählt wird, werden alle Zeichen aus `DATENTYP_E` verwendet.
...
:desc-image-aktionseingabefeld: Eingabefeld mit CharPicker
[id="image-charpicker-button",reftext="{figure-caption} {counter:figures}"]
.{desc-image-charpicker-button}
image::images/charpicker-button.png[align="center"]
...
:desc-image-aktionseingabefeld: CharPicker nach DIN SPEC 91379
[id="image-charpicker-widget",reftext="{figure-caption} {counter:figures}"]
.{desc-image-charpicker-widget}
image::images/charpicker-widget.png[align="center"]
