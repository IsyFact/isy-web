...
Die Bibliothek `isy-web` enthält alle notwendigen Abhängigkeiten, Spring-Konfigurationen, JSF-Komponenten (Composite Components) sowie zugehörige XHTML-Templates zur Erstellung der GUI-Komponente.
...
Für die Erstellung von Web-GUIs existieren Vorgaben in Form eines <<Styleguide,Styleguides>>.
...
Die CSS-Dateien für eine zum Styleguide konforme Anwendung werden bei Nutzung der Bibliothek `isy-style` automatisch geladen und eingebunden.
...
:desc-listing-SnipConfigxml: Ausschnitt webflow.xml
...
:desc-listing-SnipFacesConfigxml: Definition eines Konverters
...
:desc-listing-jsfRenderer: Konfiguration der IsyFact-spezifischen Renderer
...
Einbinden der Basiskonfiguration::
Der JSF-Baustein stellt eine Konfigurationsdatei bereit, welche die Basiskonfiguration aller verwendeten Frameworks (Spring, JSF, Spring Web Flow) bezüglich der GUI-Komponente enthält.
Diese Datei muss als Bestandteil des Applikationskontextes des Dispatcher Servlets importiert werden.

:desc-listing-PfadWebflowXml: Einbinden der Basiskonfiguration
...
----
// TODO: Stand 19.07.2018 Pfad plis-web z.Zt. noch richtig, bei einer Verzeichnisstrukturanpassung wäre das obige Beispiel anzupassen

Des Weiteren muss ein eigenes Dispatcher Servlet in der `web.xml` angelegt werden, das den Aufruf der entsprechenden URLs an Spring Web Flow weiterleitet.

:desc-listing-Snipwebxml3: Konfiguration des Dispatcher Servlets für Spring Web Flow
...
<servlet>
	...
	<init-param>
		...
		<param-value>{pfad-zur-konfiguration-des-dispatcher-servlet-kontextes}</param-value>
	</init-param>
...
:desc-demoWebGui-ApplicationTemplate: Einbindung eines Ausgangsseiten-Templates (applikation.xhtml)
[id="demoWebGui-ApplicationTemplate", reftext="{figure-caption} {counter:figures}"]
.{desc-demoWebGui-ApplicationTemplate}
image::demoWebGui-ApplicationTemplate.png[align="center"]
...
:desc-listing-jsfViewState: Beispiel für eine JSF ViewState Seite mit Inhaltsbereichsangabe
...
:desc-listing-BindViewspezJS: Einbindung viewspezifischer JavaScript-Dateien
...
Die Vorgaben zur Nutzung von JavaScript aus dem <<KonzeptJSF>> sind unbedingt einzuhalten.
...
Damit eingehende Anfragen richtig verarbeitet werden können, müssen diese zunächst mit dem korrekten Zeichensatz kodiert werden.
Um ein korrektes Kodieren zu gewährleisten, muss der `SetCharacterEncodingFilter` als erster Filter-Mapping Eintrag in der `web.xml` eingetragen sein.
Standardmäßig sollte `UTF-8` als Zeichensatz angegeben sein und, falls anderweitig gesetzt, erzwungen werden.

:desc-listing-SnipWebxmlt8: Ausschnitt aus web.xml (bezüglich SetCharacterEncodingFilter)
[id="listing-SnipWebxmlt8",reftext="{listing-caption} {counter:listings }"]
.{desc-listing-SnipWebxmlt8}
[source,xml]
----
<filter>
...
:desc-listing-SnipWebflow1xml: Ausschnitt aus webflow.xml
...
:desc-listing-SnipWebflow2xml: Ausschnitt aus webflow.xml
...
:desc-listing-maskentexteKonfigurierbar: Maskentexte über Konfigurationsdatei
...
Auf übergreifende Maskentexte aus der Konfigurationsdatei `resources/nachrichten/maskentexte.properties` kann über die Variable `msg` zugegriffen werden.
...
<<image-VerzBspErstellen>> zeigt den Verzeichnisbaum der <<Vorlageanwendung>> und verdeutlicht, wie die Elemente einer Dialog-Komponente (hier: `Erstellen`) im Dateisystem abgelegt werden.

:desc-image-VerzBspErstellen: Verzeichnisstruktur der Dialog-Komponente Erstellen
...
Das <<KonzeptLogging>> enthält die dafür notwendigen Konfigurationen und weitere Details.
...
Die Validierung in der <<glossar-GUI>>, aber spätestens im <<glossar-Anwendungskern>>, muss die fehlerhafte Eingabe feststellen.
...
Die verwendeten Rechte müssen durch die Rollenrechte-Konfiguration (siehe <<NutzungsvorgabenSicherheit>>) für den eingeloggten Benutzer entsprechend gesetzt werden.
...
Der Security Filter benötigt einen `ContextLoaderListener`, welcher wiefolgt konfiguriert ist.
...
Die eingebundenen Spring-Konfigurationen erwarten eine Bean unter dem Namen "accessDecisionManager"vorzufinden.
Die Bean-Attribute "property name" und "ref" sind hierbei auf den String "sicherheit" zu setzen.

:desc-listing-Berechtigungsmanager: Bean Berechtigungsmanager
[id="listing-Berechtigungsmanager",reftext="{listing-caption} {counter:listings }"]
.{desc-listing-Berechtigungsmanager}
[source,xml]
----
    <bean id="accessDecisionManager"
          class="de.bund.bva.isyfact.sicherheit.web.DelegatingAccessDecisionManager">
...
Für die Anbindung des Berechtigungsmanagers an Spring Security findet die Möglichkeit zur Erstellung eines “Custom Authentication Providers” Anwendung.
In diesem Authentication Provider wird bei Bedarf überprüft ob der Nutzer die notwendigen Berechtigungen besitzt, bzw. es wird für die weitere Verarbeitung in Spring Security der notwendige Kontext aufgebaut.
...
Hierbei werden die Rollen eines Benutzers über den Berechtigungsmanager gelesen und in einen Spring Security konformen Token geschrieben.
...
====================================================================== -->
<beans:bean id="WebFlowAuthenticationProvider"
	class="de.bund.bva.pliscommon.plisweb.WebFlow.security.WebFlowAuthenticationProvider">
...
==== Konfiguration von Spring (webflow.xml)

Hier muss ein Listener registriert werden, über welchen die Security Bedürfnisse geprüft werden.

:desc-listing-SnipWebFlowxx: Ausschnitt aus webflow.xml (bezüglich Security)
...
<bean name="flowExecutionListenerLoaderFactory"
	  class="org.springframework.webflow.config.FlowExecutionListenerLoaderFactoryBean">
...
Für den Schutzbedarf *mittel* und *hoch* reicht die Implementierung der Variante _Token per Session_, für Schutzbedarf *sehr hoch* muss die Varainte _Token per Request_ implementiert werden (vgl. <<KonzeptJSF>>).
...
:desc-listing-hiddenInput: Verwendung eines hiddenInput Elements für die CSRF-Token Übertragung
[id="listing-hiddenInput",reftext="{listing-caption} {counter:listings }"]
.{desc-listing-hiddenInput}
[source,html]
----
<!-- [...] -->
<h:form>
...
:desc-listing-isySicherheitWeb: Ausschnitt aus isy-sicherheit-web.xml: Verwendung eines eigenen CsrfTokenRepositories
[id="listing-isySicherheitWeb",reftext="{listing-caption} {counter:listings }"]
.{desc-listing-isySicherheitWeb}
[source,html]
----
<!-- [...] -->
...
image::Browser_Druckfunktion.png[align="center", scaledwidth="96%"]
...
image::Aufbau_Applikationsseite.png[align="center", width="629",height="526"]
...
Der Header-Bereich enthält allgemeine Informationen zur <<glossar-Anwendungslandschaft>>, den in ihr verfügbaren Fachanwendungen und zum angemeldeten Anwender.
...
<<glossar-Fachanwendung,Fachanwendungen>> können globale Standardwerte definieren.
...
:desc-listing-zusatzmaskentitel: Zusatz-Maskentitel im Header-Bereich
...
:desc-image-modaler-dialog: Beispiel eines modalen Dialogs
[id="image-modaler-dialog",reftext="{figure-caption} {counter:figures}"]
.{desc-image-modaler-dialog}
image::demoflow-modaler-dialog.png[align="center"]
...
:desc-image-bestaetigungs-dialog: Beispiel eines Bestätigungsdialogs (Popup-Dialog)
[id="image-bestaetigungs-dialog",reftext="{figure-caption} {counter:figures}"]
.{desc-image-modaler-dialog}
image::demoflow-bestaetigungs-dialog.png[align="center"]
...
:desc-listing-Bestaetigungsdialog: Bestätigungsdialog - subflow mit Parametern
[id="listing-Bestaetigungsdialog",reftext="{listing-caption} {counter:listings }"]
.{desc-listing-Bestaetigungsdialog}
[source, xml]
----
<?xml version="1.0" encoding="UTF-8"?>
...
image::Wizard_Beispiel_Schritt_2.png[align="center"]
...
:desc-listing-wizardModaleDialoge: Modaler Wizard
[id="listing-wizardModaleDialoge",reftext="{listing-caption} {counter:listings }"]
.{desc-listing-wizardModaleDialoge}
[source, xml]
----
...
image::Menuepunkt_Flows.png[align="center"]
...
image::Kein_Javascript_Wizard.png[align="center"]
...
Die Anwendung zeigt die Features der <<glossar-Bibliothek,Bibliotheken>> `isy-web` sowie `isy-style` beispielhaft.
...
Wird der Wert auf `dinNorm` gesetzt, wird der Character-Picker verwendet, welcher alle Zeichen der DIN NORM 91379 unterstützt. Für Rückwärtskompatibilität kann weiterhin der Wert `dinSpec` verwendet werden.
...
Dazu muss, wie im folgenden Beispiel (<<listing-charpicker>>), der Parameter `charpicker` auf den Wert `dinNorm` gesetzt werden, um den CharPicker zu verwenden, welcher die Sonderzeichen der DIN NORM 91379 unterstützt:

:desc-listing-tabgroup: Verwendung vom CharPicker
[id="listing-charpicker",reftext="{listing-caption} {counter:listings }"]
...
:desc-image-charpicker-button: Eingabefeld mit CharPicker
[id="image-charpicker-button",reftext="{figure-caption} {counter:figures}"]
.{desc-image-charpicker-button}
image::charpicker-button.png[align="center"]
...
:desc-image-charpicker-widget: CharPicker nach DIN NORM 91379
[id="image-charpicker-widget",reftext="{figure-caption} {counter:figures}"]
.{desc-image-charpicker-widget}
image::charpicker-widget.png[align="center"]
...
* `globalConfig`: Spezifische, globale Konfiguration. (Standard: `globalConfigurationModel`)
* `charpicker`: Gibt an, welche Eingabe für Sonderzeichen verwendet werden soll.
Wird der Wert auf `dinNorm` gesetzt, wird der neue Character-Picker verwendet, welcher alle Zeichen der DIN NORM 91379 unterstützt. Für Rückwärtskompatibilität kann weiterhin der Wert `dinSpec` verwendet werden.
Soll der alte Legacy-Character-Picker verwendet werden, muss der Wert auf `special` gesetzt werden.
Standardmäßig wird kein Character-Picker verwendet.
...
image::Aktionseingabefeld.png[align="center"]
...
image::Textbox.png[align="center", pdfwidth=60%]
...
image::Dropdown_Menue_Flyout.png[align="center"]
...
.{desc-image-bildtabs}
image::Tabs_Beispiel.png[align="center"]
...
image::Liste_mit_Listenkopf.png[align="center",pdfwidth=60%]
...
:desc-listing-listpickerMitServlet: Listpicker mit Servlet-Filterung
[id="listing-listpickerMitServlet",reftext="{listing-caption} {counter:listings }"]
.{desc-listing-listpickerMitServlet}
[source, xml]
----
<isy:formListpicker
  reference="anfrageIdentifikation.gruppe"
...
Zweitens muss der Wert des Parameters sich in der Datei web.xml wiederfinden.

:desc-listing-beispielEinbindungServlet: Beispielhafte Einbindung in web.xml
[id="listing-beispielEinbindungServlet",reftext="{listing-caption} {counter:listings }"]
.{desc-listing-beispielEinbindungServlet}
[source, xml]
----
<servlet>
  <servlet-name>listpickerprovider</servlet-name>
...
<servlet-mapping>
	<servlet-name>listpickerprovider</servlet-name>
	<url-pattern>/app/listpicker/*</url-pattern>
</servlet-mapping>
----

Der Parameter `servletUrl` hat hier beispielsweise den Wert "/app/listpicker" und das Servlet erfasst laut Eintrag in der web.xml alle Requests, deren URL auf das Pattern "/app/listpicker/*" passen.
...
In dem obigen Beispiel heißt die Bean der Implementierung `listpickerprovider`.
...
:desc-listing-listpickerMitServletWeitererParam: Servlet-URL mit weiteren Filter-Parametern
[id="listing-listpickerMitServletWeitererParam",reftext="{listing-caption} {counter:listings }"]
.{desc-listing-listpickerMitServletWeitererParam}
[source, xml]
----
<isy:formListpicker
	....
...
image::Tabelle_Einfuehrung.png[align="center"]
...
image::Tabelle_Zustaende_Zeilen.png[align="center"]
...
:desc-image-tabellesortierungimkopf: Tabelle – Sortierungs-Indikator im Tabellenkopf
...
image::Tabelle_Mehrfachselektion.png[align="center"]
...
image::Tabelle_Mehrfach_Checkbox.png[align="center"]
...
image::Textlink_Kennzeichnung_Objekt.png[align="center"]
...
image::Tabelle_Button_MehrAnzeigen.png[align="center"]
...
image::RadioButton_Normal_Disabled.png[align="center"pdfwidth=30%]
...
image::horizontale-navigation-submenu.png[align="center", width="60%", pdfwidth="60%"]
...
image::Quicklinks_LetzteObjekte.png[align="center",pdfwidth=30%]
...
image::Quicklinks_Wiedervorlage.png[align="center",pdfwidth=30%]
...
:desc-listing-hilfe-konfig: Konfiguration des Hilfe-Buttons
[id="listing-hilfe-konfig",reftext="{listing-caption} {counter:listings }"]
.{desc-listing-hilfe-konfig}
[source,properties]
----
...
:desc-listing-hilfe-deaktivieren: Ausschalten des Hilfe-Buttons
[id="listing-hilfe-deaktivieren",reftext="{listing-caption} {counter:listings }"]
.{desc-listing-hilfe-deaktivieren}
[source,properties]
----
...
:desc-listing-hilfe-flow: Ausschalten des Hilfe-Buttons in einem Flow
[id="listing-hilfe-flow",reftext="{listing-caption} {counter:listings }"]
.{desc-listing-hilfe-flow}
[source,xml]
----
<on-start>
...
Die einzelnen Filter sollten als <f:selectItem> angegeben werden.
...
[TIP]
====
Die IsyFact enthält die Demo-Anwendung `isy-webgui` und zeigt darin die Features der Bibliotheken `isy-web` sowie `isy-style` beispielhaft.
...
* `validationModel`: Ein spezifisches Validation-Model, falls benötigt. (Standard: `#{validationModel}`)
* `globalConfig`: Eine spezifische globale Konfiguration, falls benötigt. (Standard: `#{globalConfigurationModel}`)
* `charpicker`: Gibt an, welche Eingabe für Sonderzeichen verwendet werden soll. Wird der Wert auf `dinNorm` gesetzt, wird der neue Character-Picker verwendet, welcher alle Zeichen der DIN NORM 91379 unterstützt. Für Rückwärtskompatibilität kann weiterhin der Wert `dinSpec` verwendet werden.
Soll der alte Legacy-Character-Picker verwendet werden, muss der Wert auf `special` gesetzt werden.
Standardmäßig wird kein Character-Picker verwendet.
