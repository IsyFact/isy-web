...
Konfiguration von Widget-Renderern (`faces-config.xml`)::
...
TIP: Die Session-Persistierung erfolgt neben den Dialogschritten zusätzlich auch beim Redirect im Rahmen der Anwendung des Post Redirect Get-Pattern (siehe <<back-button-handling>>).
...
Gibt es View-übergreifende Funktionalität kann diese in eine wiederverwendbare JavaScript-Datei ausgelagert werden (z.B. `erstellenForumlar.js`).
...
Für die Verwendung der Widgets muss in den XHTMLs folgender Namespace eingebunden werden: `xmlns:isy="http://java.sun.com/jsf/composite/isyfact"`
...
Default ist false, so dass nicht vorgeladen wird. (Standard: `false`, Typ: `java.lang.Boolean`)
...
Entscheidend ist der Paramater `servletUrl`.
...
Die eigentliche Filterungslogik obliegt dann der konkreten Implementierung.
Den Wert des Filter-Feldes erhält sie jedoch von der abstraken Oberklasse.
