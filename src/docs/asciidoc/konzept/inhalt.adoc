...
Das zentrales Konzept hinter JSF ist die Erstellung eines Komponentenbaumes aus diesen grafischen Komponenten.
...
Daher wurde diese Option in der <<IsyFact-Referenzarchitektur>> ausgeschlossen.
...
Der Inhalt der Variable `searchString` wird ersetzt, so dass folgender Code ausgeführt wird:
...
Geschieht dies nicht, bestaht das Sicherheitsrisiko von Ausführung von beliebigem Code (z.B. durch Request-Strings übergeben) auf dem Server.
