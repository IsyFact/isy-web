...
Für detaillierte Informationen über die verwendeten Technologien sei auf entsprechende Literatur verwiesen: <<WikiJSF>>, <<SWF>> und <<Spring>>.
...
Die IsyFact setzt die Referenzimplementierung von Oracle (<<Mojarra>>) ein und erweitert diese im Bereich der Komponenten um die Bibliothek Apache Tomahawk (<<Tomahawk>>).

*Facelets* ist eine Template-Engine und die Standard-View-Technologie von JSF.
Facelets löst in dieser Funktion JSP ab.
Mit Facelets werden kleine, wiederverwendbare <<glossar-GUI>>-Komponenten erstellt, die durch ein entsprechendes XML-Tag in eine Seite inkludiert werden.
...
Als Implementierung kommt wiederum die Referenzimplementierung von Oracle (<<Mojarra>>) zum Einsatz.
...
Siehe auch Kapitel "Benötigte Bibliotheken" in <<NutzungsvorgabenJSF>>.
...
Die <<glossar-GUI>>-Komponente und der <<glossar-Anwendungskern>> sind Teil derselben Web-Applikation und werden per Spring-Konfiguration miteinander verbunden.

Oft gibt es den Fall, dass über die GUI eine Aktion in einer anderen Anwendung ausgelöst werden soll.
Ein Beispiel dafür ist die GUI einer <<glossar-Fachanwendung>> zur Datenerfassung, wobei die Speicherung der Daten über einen Service einer anderen Fachanwendung implementiert ist.
...
Daher wurde diese Option in der <<IsyFactReferenzarchitektur>> ausgeschlossen.
...
JQuery ist ein JavaScript-Framework, das auf einfache Weise JavaScript-Funktionen bereitstellt, die insbesondere auf die grafische Gestaltung einer Oberfläche benötigt werden.
...
Die Architektur von <<glossar-GUI,GUIs>> mit JSF und Spring Web Flow hält sich an die Vorgaben aus der <<IsyFactReferenzarchitektur>> sowie die architektonischen Vorgaben aus dem <<DetailkonzeptKomponenteWebGUI>>.
...
* Verwaltung von Sessions über Spring Web Flow
...
:desc-image-aufbau-gui-komponente: Aufbau einer Web-GUI mit JSF und Spring Web Flow
[id="image-aufbau-gui-komponente",reftext="{figure-caption} {counter:figures}"]
.{desc-image-aufbau-gui-komponente}
image::aufbau-gui-komponente.png[align="center"]
...
Wie im <<DetailkonzeptKomponenteWebGUI>> beschrieben, besteht die GUI-Komponente aus Dialog-Komponenten und einem gemeinsamen AWK-Wrapper.
...
:desc-image-innensicht-gui-komponente: Innensicht einer GUI-Komponente mit ihren Dialog-Komponenten
[id="image-innensicht-gui-komponente",reftext="{figure-caption} {counter:figures}"]
.{desc-image-innensicht-gui-komponente}
image::innensicht-gui-komponente.png[align="center"]
...
:desc-image-kommunikation-innerhalb-gui-komponente: Kommunikation innerhalb einer Dialog-Komponente
[id="image-kommunikation-innerhalb-gui-komponente",reftext="{figure-caption} {counter:figures}"]
.{desc-image-kommunikation-innerhalb-gui-komponente}
image::kommunikation-innerhalb-gui-komponente.png[align="center"]
...
Es hat keine Abhängigkeiten zu View, Controller oder <<glossar-Anwendungskern>> und enthält im Regelfall keine Logik.
...
Der Abgleich von Model Beans mit einer View und das Abspeichern der Daten einer Model Bean wird im Kapitel "Erstellung der Models" in <<NutzungsvorgabenJSF>> genauer beschrieben.
...
Die View-Erstellung wird im Kapitel "Erstellung der Views" in <<NutzungsvorgabenJSF>> genauer beschrieben.
...
Es ist wichtig, dass eine Kopie übergeben wird, damit Flow B nicht Teile des Model A absichtlich oder versehentlich ändert.
...
Für die Steuerung des Vorgabelayouts (z.B. Menüleiste, Linksnavigation) sowie der Nutzung von vorgegebenen Funktionen (z.B. Validierung) werden querschnittliche Controller mit zugehörigen Models verwendet.
...
Welche Controller im Detail (LinksnavigationController, QuicklinksController, ValidierungsController, ...) für das Vorgabelayout verfügbar sind, wird im Dokument <<NutzungsvorgabenJSF>> beschrieben.
...
Für die Arbeit mit JSF werden gemäß dem <<Styleguide>> sowohl Seitenelemente als auch Bedienelemente bereitgestellt.
...
Die Nutzung der durch den Styleguide definierten Elemente und durch den Baustein bereitgestellten JSF-Komponenten ist ein zentrales Thema der <<NutzungsvorgabenJSF>>.

[[absicherung-von-jsf-guis]]
==== Absicherung von JSF-GUIs

Die Absicherung von Masken erfolgt auf Ebene des Dialogablaufs, also von Spring Web Flow.
Die Berechtigungsprüfung verwendet den Baustein Sicherheit der IsyFact.

Für die Anbindung des dort definierten `Berechtigungsmanager` an Spring Security findet die Möglichkeit zur Erstellung eines “Custom Authentication Providers” in Form des `WebFlowAuthenticationProvider` Anwendung.
Der `WebFlowAuthenticationProvider` prüft bei Bedarf, ob der Anwender die notwendigen Berechtigungen besitzt, bzw. es wird für die weitere Verarbeitung in Spring Security der notwendige Kontext aufgebaut.
<<image-integration-baustein-sicherheit-klassendiagramm>> zeigt die Integration des Bausteins Sicherheit in Spring Security.

:desc-image-integration-baustein-sicherheit-klassendiagramm: Integration des Bausteins Sicherheit in Spring Security
[id="image-integration-baustein-sicherheit-klassendiagramm",reftext="{figure-caption} {counter:figures}"]
.{desc-image-integration-baustein-sicherheit-klassendiagramm}
image::integration-baustein-sicherheit-klassendiagramm.png[align="center"]

<<image-integration-baustein-sicherheit-sequenzdiagramm>> stellt den Zugriff des `WebFlowAuthenticationProvider` auf den Baustein Sicherheit dar.
...
:desc-image-integration-baustein-sicherheit-sequenzdiagramm: Sequenzdiagramm Zugriff auf Sicherheitskomponente
[id="image-integration-baustein-sicherheit-sequenzdiagramm",reftext="{figure-caption} {counter:figures}"]
.{desc-image-integration-baustein-sicherheit-sequenzdiagramm}
image::integration-baustein-sicherheit-sequenzdiagramm.png[align="center"]

Mehr Details hierzu sind in der Bausteindokumentation (<<KonzeptSicherheit>> und <<NutzungsvorgabenSicherheit>>) sowie in den <<NutzungsvorgabenJSF>> zu finden.
...
:desc-listing-ConvDatewihConv: Umwandlung eines Datums mittels Standard-JSF-Konverter
...
Die Konvertierung findet in diesem Fall nicht durch einen Konverter statt, sondern erst während oder nach der Validierung an der Schnittstelle zum <<glossar-Anwendungskern>>.

[[serverseitige-validierung-von-eingaben]]
==== Serverseitige Validierung von Eingaben

Die Validierung und Prüfung der in der GUI erfassten Daten soll entweder vollständig durch die GUI oder aber vollständig im <<glossar-Anwendungskern>> durchgeführt werden.
Die Validierung in der GUI ist dabei bevorzugt.
In diesem Falle wird der Validierungsmechanismus von Spring Web Flow verwendet (s. Abschnitt 5.10 der <<SWF>>).
...
[ARCHITEKTURREGEL]
====
*Nutzung von anonymen Funktion anstatt benannter Funktionen*
...
Weitere Erläuterungen zum Session-Manager finden sich im <<KonzeptSessionManagement>>,
Die Einbindung des Session-Managers ist in <<NutzungsvorgabenIsySession>> beschrieben.
Konkrete Anweisungen zum Aufbau des Session-Speichers finden sich in <<NutzungsvorgabenRedis>>.


Wenn `ìsy-session` als Erweiterung der IsyFact nicht eingesetzt wird, muss die Session-Persistierung auf andere Weise durchgeführt
