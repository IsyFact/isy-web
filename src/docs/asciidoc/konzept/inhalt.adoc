...
Mit Facelets werden kleine, wiederverwendbare GUI-Komponenten erstellt, die durch ein entsprechendes XML-Tag in eine Seite inkludiert werden.
...
Aus Sicht der Komponente GUI bedeutet dies, dass jeder Aufruf des Anwendungskerns unmittelbar eine Änderung der Daten zufolge hat.
...
[[architektur]]
== Architektur der JSF-Web-GUI

Die Architektur von Web-GUIs mit JSF und Spring Web Flow hält sich an die Vorgaben aus der <<IsyFactReferenzarchitektur>> sowie die architektonischen Vorgaben aus dem <<DetailkonzeptKomponenteWebGUI>>.
...
Die Konvertierung findet in diesem Fall nicht durch einen Konverter statt, sondern erst während oder nach der Validierung an der Schnittstelle zum Anwendungskern.
...
Die Applikation muss auf die Vorgaben des <<Styleguide,Styleguides>> überprüft werden.
...
Die unter <<vorgaben-javascript>> und <<vermeidung-von-sicherheitsluecken-bei-aktiviertem-javascript>>
...
Die entsprechenden Regeln sind im <<KonzeptSicherheit>> beschrieben.
// end::inhalt[]