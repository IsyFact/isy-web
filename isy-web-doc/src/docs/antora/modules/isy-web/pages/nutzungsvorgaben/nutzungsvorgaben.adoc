...
|====
:linkaktuell: die https://isyfact.github.io/isy-angular-widgets/documentation/[Dokumentation der Komponenten-Bibliothek icon:external-link[]^] des Bausteins Angular
...
Im Dokument xref:3.2.1@isyfact-standards-doku:blaupausen:referenzarchitektur-it-system/master.adoc[Softwaretechnische Architektur] ist der Aufbau von xref:glossary::terms-definitions.adoc#glossar-it-system[IT-Systemen] in fünf Komponenten beschrieben.
...
Die Nutzungsvorgaben bauen auf dem xref:3.2.1@isyfact-standards-doku:blaupausen:detailkonzept-komponente-web-gui/master.adoc[Detailkonzept Web-GUI] sowie dem xref:konzept/konzept.adoc[Konzept JSF] auf und beschreiben die Umsetzung der GUI-Komponente mit den Technologien JSF und Spring Web Flow.
...
Für die Erstellung von Web-GUIs existieren Vorgaben in Form eines xref:isy-bedienkonzept-doc:ROOT:bedienkonzept.adoc[Bedienkonzepts].
Der Baustein Bedienkonzept legt Anforderungen an die Gestaltung und Nutzbarkeit von Oberflächen fest.
Die Bibliothek `isy-web` enthält die CSS-, JavaScript-, Bilddateien und  Schriftarten des Bedienkonzepts zur produktiven Nutzung aufbereitet.

Die CSS-Dateien für eine zum Bedienkonzept konforme Anwendung werden bei Nutzung der Bibliothek `isy-web` automatisch geladen und eingebunden.
...
[[enthaltene-bibliotheken-plugins-skripte]]
=== Enthaltene Bibliotheken, Plugins und Skripte

Um eine umfassendere Funktionalität zu gewährleisten, muss teilweise zusätzliches JavaScript eingesetzt werden.
Diese Skripte dienen beispielsweise zum Styling applikationsspezifischer Bedienelemente, Optimieren der Browser-Kompatibilität oder Erweiterung der Standardinteraktivität.
...
Implementiert einen interaktiven https://github.com/uxsolutions/bootstrap-datepicker[Standard Date Picker]

*Bootstrap Select 1.13.18*

Implementiert eine https://developer.snapappointments.com/bootstrap-select/[Auswahlbox]

*Bootstrap Timepicker 0.3.0*

Implementiert einen interaktiven https://github.com/jdewit/bootstrap-timepicker/[Standard Time Picker]
...
Dieser enthält anwendungsübergreifend einheitliche Vorgaben zu global gültigen Regeln, Fehler-Handler und Layout-Konfigurationen.
...
Der Controller ist der „verlängerte Arm“ des Flows, denn im Flow darf keine GUI-Logik ausgeführt werden.
...
Der Controller wird in der `ControllerAutoConfiguration` als einfaches Spring-Bean definiert und ist somit im Flow automatisch sichtbar und nutzbar.
...
Eine Rückgabe von Zielzuständen zur Steuerung des Flows in Methoden des Controllers ist zu vermeiden.
...
Das Tag `<ui:composition>` referenziert hierbei das Seitentemplate, in welchem die über `<ui:define>` definierten Teile eingebunden werden.
...
Es ist wichtig zu verstehen, dass durch das Rendern der Daten aus dem Model eine Bindung der Model-Property mit dem GUI-Element (im Beispiel ein Form-Input-Feld) hergestellt wird, welches nach Submit des Webformulars automatisch durch Spring Web Flow in das Model zurück synchronisiert wird.
...
.Einbindung view-spezifischer JavaScript-Dateien
[source,xml]
----
...
In der Regel bieten die <<jsf-bedienelemente>> der IsyFact Zugriff auf Darstellungsformen mittels JavaScript (z.B. Kalenderwidget, Tags, Panels).
...
Zur Autokonfiguration von Controllern, WebMVC und Flows sind hierbei die Klassen `ControllerAutoConfiguration`, `MvcAutoConfiguration` und `WebFlowAutoConfiguration` vorgesehen und müssen importiert werden.
...
Dieses ist notwendig, weil eine Angabe der Flow Engine für Welcome Files nicht direkt möglich ist.
...
Standardmäßig sollte hier keine spezifische Konfiguration vorgenommen werden, eine Autokonfiguration findet bereits in `WebFlowAutoConfiguration` statt.


[[ablage-der-konfiguration]]
==== Ablage der Flow-Konfiguration

Die Spring Webflow Konfiguration erfolgt standardmäßig in der Autokonfiguration.
Als Lokation der Flows ist dabei `/WEB-INF/gui/**/*Flow.xml` angegeben.
Das heißt, die Flows sind in Unterordnern von `/WEB-INF/gui` abzulegen und enden gemäß Namenskonvention mit `Flow.xml` (z.B. `beispielFlow.xml`).
...
Für die Namenskonvention zu Java-Klassen und Paketen wird hier auf das Dokument xref:3.2.1@isyfact-standards-doku:methodik:java-programmierkonventionen/master.adoc[Java-Programmierkonventionen] verwiesen.
...
Die Abbildung  <<image-VerzBspErstellen>> zeigt einen Verzeichnisbaum und verdeutlicht, wie die Elemente einer Dialog-Komponente (hier: `Erstellen`) im Dateisystem abgelegt werden.
...
Das xref:3.2.1@isyfact-standards-doku:isy-logging:konzept/master.adoc#einleitung[Konzept Logging] enthält die dafür notwendigen Konfigurationen und weitere Details.
...
Die xref:3.2.1@isyfact-standards-doku:isy-logging:nutzungsvorgaben/master.adoc#einleitung[Nutzungsvorgaben Logging] beinhalten Vorgaben zum Schreiben von Log-Einträgen und zur Ablage von Log-Dateien.
...
Die Darstellung von Fehlern und Validierungsnachrichten wird auch im xref:isy-bedienkonzept-doc:ROOT:bedienkonzept.adoc[Bedienkonzept] beschrieben.
...
Hierzu finden sich weitere Informationen im Dokument xref:3.2.1@isyfact-standards-doku:isy-exception-core:konzept/master.adoc[Konzept Fehlerbehandlung].
...
Die verwendeten Rechte müssen durch die Rollenrechte-Konfiguration (siehe xref:3.2.1@isyfact-standards-doku:isy-security:nutzungsvorgaben/master.adoc[Nutzungsvorgaben Security]) für den eingeloggten Benutzer entsprechend gesetzt werden.
...
Dieser Controller ist dafür zuständig, den Rechte-Schlüssel aufzulösen und die Anfrage an die Komponente Security zu delegieren.
Beantwortet werden die Anfragen stets mit einem einfachen Booleschen Wert.

Mithilfe dieses Controllers lassen sich einzelne Maskenelemente, aber auch Gruppen von Maskenelementen berechtigungsabhängig ausblenden.
...
Ist JavaScript in einem Browser aktiviert, eröffnet dies gewisse Risiken bei der Verarbeitung datenschutzrelevanter Informationen.
...
[[konfiguration-securitykomponente]]
===== Konfiguration Security-Komponente

Die Konfiguration der Security-Komponente ist in den xref:3.2.1@isyfact-standards-doku:isy-security:nutzungsvorgaben/master.adoc[Nutzungsvorgaben Security] beschrieben.
...
Für den Schutzbedarf *mittel* und *hoch* reicht die Implementierung der Variante _Token per Session_, für Schutzbedarf *sehr hoch* muss die Variante _Token per Request_ implementiert werden (vgl. xref:konzept/konzept.adoc[Konzept JSF]).
...
Im xref:isy-bedienkonzept-doc:ROOT:bedienkonzept.adoc[Bedienkonzept] finden sich Beispiele für Druckansichten.
...
Die Einbindung erfolgt dann per Dependency Injection z.B. in einem Controller.
...
Der Header-Bereich enthält allgemeine Informationen zur xref:glossary::terms-definitions.adoc#glossar-anwendungslandschaft[Anwendungslandschaft], den in ihr verfügbaren Fachanwendungen und zum angemeldeten Anwender.
...
xref:glossary::terms-definitions.adoc#glossar-geschaeftsanwendung[Geschäftsanwendungen] können globale Standardwerte definieren.
...
[start=2]
. *Maven Install*: In dem Modul `isy-web-lib` ist ein "Maven install" durchzuführen.
Dadurch wird ein "webpack build" ausgelöst und die Farbstyles befinden sich nach der Generierung in `target/classes/META-INF/resources/css/color.css`.

. *Einbindung in Zielanwendung*: Die Datei `color.css` aus dem Target-Verzeichnis von `isy-web-lib` ist in die Zielanwendung unterhalb des WEB-INF Verzeichnisses zu kopieren.
...
.CharPicker nach DIN NORM 91379
...
Für den Datepicker wird ein https://bootstrap-datepicker.readthedocs.io[externes Plug-In] verwendet.
...
.Demo-Anwendung: Dropdown Menü (außerhalb einer Form)
...
Falls JavaScript nicht aktiviert ist, dann wird statt des List Pickers ein einfaches <<dropdown-menue>> angezeigt.
...
Dieses Tag sollte nach den Headern in den Body der Datatable geschrieben werden.
...
* `dropdown`: Gibt an, ob eine Auswahlliste zur Filterung angezeigt werden soll.
Falls ja, müssen die Einträge mittels selectItem(s) angegeben werden. (Standard: `false`, Typ: `Boolean`)
...
Im Modus `SERVER` wird die Methode `DataTableController.updateDisplayItems(...)` aufgerufen, welche die Tabelleneinträge filtert, sortiert und anschließend seitenweise anzeigt.
...
.Tabelle mit "mehr zeigen" Button
...
Hierzu muss jede Anwendung zunächst eine Instanz von `de.bund.bva.isyfact.common.web.jsf.components.navigationmenu.generieren.impl.NavigationMenuGenerierenAusKonfiguration` im Application-Context zur Verfügung stellen und korrekt konfigurieren.
Die Bean benötigt lediglich einen `de.bund.bva.isyfact.security.core.Berechtigungsmanager` und eine `de.bund.bva.isyfact.konfiguration.common.Konfiguration`, um korrekt zu funktionieren.
...
* `headerPanel`: Gibt an, ob die Gruppe als Container für den Header des Inhaltsbereichs dient.
Falls ja, werden entsprechende CSS-Klassen für das Layouting eingebunden. (Standard: `false`, Typ: `java.lang.Boolean`)
...
Insbesondere demonstriert die Demo-Anwendung Aussehen und Verhalten der nachfolgend beschriebenen JSF-Seitenelemente.
Alle Screenshots der beschriebenen 4-Augen-Prinzip Komponenten wurden dieser Demo-Anwendung entnommen.
====
...
.Demo-Anwendung: isy_formActionInput
...
[id="demopanel-formCurrencyInput.png",reftext={figure-caption}{counter:figures}]
...
.Demo-Anwendung: isy_formDateWithFourEyes Varianten
...
.Demo-Anwendung: isy_formInputWithFourEyes Varianten
...
.Demo-Anwendung: isy_formNumericInputWithFourEyes Varianten
...
.Demo-Anwendung: isy_formSelectOneDropdownWithFourEyes Varianten
...
.Demo-Anwendung: isy_formTextareaWithFourEyes
...
.Demo-Anwendung: isy_selectOneDropdownWithfourEyes
