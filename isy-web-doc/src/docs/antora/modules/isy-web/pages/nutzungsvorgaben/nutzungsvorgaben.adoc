...
Im Dokument xref:3.3.0@isyfact-standards-doku:blaupausen:referenzarchitektur-it-system/master.adoc[Softwaretechnische Architektur] ist der Aufbau von xref:glossary::terms-definitions.adoc#glossar-it-system[IT-Systemen] in fünf Komponenten beschrieben.
...
Die Nutzungsvorgaben bauen auf dem xref:3.3.0@isyfact-standards-doku:blaupausen:detailkonzept-komponente-web-gui/master.adoc[Detailkonzept Web-GUI] sowie dem xref:konzept/konzept.adoc[Konzept JSF] auf und beschreiben die Umsetzung der GUI-Komponente mit den Technologien JSF und Spring Web Flow.
...
Für die Namenskonvention zu Java-Klassen und Paketen wird hier auf das Dokument xref:3.3.0@isyfact-standards-doku:methodik:java-programmierkonventionen/master.adoc[Java-Programmierkonventionen] verwiesen.
...
Das xref:3.3.0@isyfact-standards-doku:isy-logging:konzept/master.adoc#einleitung[Konzept Logging] enthält die dafür notwendigen Konfigurationen und weitere Details.
...
Die xref:3.3.0@isyfact-standards-doku:isy-logging:nutzungsvorgaben/master.adoc#einleitung[Nutzungsvorgaben Logging] beinhalten Vorgaben zum Schreiben von Log-Einträgen und zur Ablage von Log-Dateien.
...
Hierzu finden sich weitere Informationen im Dokument xref:3.3.0@isyfact-standards-doku:isy-exception-core:konzept/master.adoc[Konzept Fehlerbehandlung].
...
Die verwendeten Rechte müssen durch die Rollenrechte-Konfiguration (siehe xref:3.3.0@isyfact-standards-doku:isy-security:nutzungsvorgaben/master.adoc[Nutzungsvorgaben Security]) für den eingeloggten Benutzer entsprechend gesetzt werden.
...
Die Konfiguration der Security-Komponente ist in den xref:3.3.0@isyfact-standards-doku:isy-security:nutzungsvorgaben/master.adoc[Nutzungsvorgaben Security] beschrieben.
