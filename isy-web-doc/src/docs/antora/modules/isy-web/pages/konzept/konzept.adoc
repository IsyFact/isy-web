...
Die Berechtigungsprüfung verwendet dabei den in beschriebenen xref:isyfact-standards-doku:isy-security:konzept/master.adoc[Baustein Security der IsyFact].
...
In der Implementierung wird mittels einer Abfrage an `Berechtigungsmanager` aus dem Security-Baustein geprüft, ob ein Anwender die notwendigen Berechtigungen besitzt (siehe dazu xref:isyfact-standards-doku:isy-security:nutzungsvorgaben/master.adoc[Nutzungsvorgaben Security]).
