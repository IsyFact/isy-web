...
Dieses Konzept baut auf dem xref:3.3.0@isyfact-standards-doku:blaupausen:detailkonzept-komponente-web-gui/master.adoc[Detailkonzept Web-GUI] auf und beschreibt die Architektur der GUI-Komponente, umgesetzt mit den Technologien JSF und Spring Web Flow.
...
* Anbindung des Anwendungskerns (Spring Beans)

*Einfachheit des Session-Managements:* Hier wird definiert, wie die Behandlung von Session Informationen erfolgen soll und welche Möglichkeiten dazu zur Verfügung gestellt werden.
...
Ein Beispiel dafür ist die GUI einer xref:glossary::terms-definitions.adoc#glossar-geschaeftsanwendung[Geschäftsanwendung] zur Datenerfassung, wobei die Speicherung der Daten über einen xref:glossary::terms-definitions.adoc#service[Service] einer anderen Fachanwendung implementiert ist.
...
Daher wurde diese Option in der xref:3.3.0@isyfact-standards-doku:blaupausen:referenzarchitektur/master.adoc[Referenzarchitektur] ausgeschlossen.
...
Die Architektur von GUIs mit JSF und Spring Web Flow hält sich an die Vorgaben aus der xref:3.3.0@isyfact-standards-doku:blaupausen:referenzarchitektur/master.adoc[Referenzarchitektur] sowie die architektonischen Vorgaben aus dem xref:3.3.0@isyfact-standards-doku:blaupausen:detailkonzept-komponente-web-gui/master.adoc[Detailkonzept Web-GUI].
...
Wie im xref:3.3.0@isyfact-standards-doku:blaupausen:detailkonzept-komponente-web-gui/master.adoc[Detailkonzept Web-GUI] beschrieben, besteht die GUI-Komponente aus Dialog-Komponenten und einem gemeinsamen AWK-Wrapper.
...
Das Controller-Bean ist ein vom Komponenten-Flow aufzurufendes *zustandsloses* Spring Bean, welches Änderungen an den Daten des Models vornimmt oder diese aufbereitet bzw. Schnittstellen des Anwendungskern-Wrappers aufruft.
...
Die Berechtigungsprüfung verwendet dabei den xref:3.3.0@isyfact-standards-doku:isy-security:konzept/master.adoc[Baustein Security].
...
In der Implementierung wird mittels einer Abfrage an `Berechtigungsmanager` aus dem Security-Baustein geprüft, ob ein Anwender die notwendigen Berechtigungen besitzt (siehe dazu xref:3.3.0@isyfact-standards-doku:isy-security:nutzungsvorgaben/master.adoc[]).
