...
Allgemeine Angaben zur Gestaltung von Oberflächen finden sich im xref:isy-style-doc:isy-style:styleguide.adoc[Styleguide].
...
* Entwickler müssen sich mit den TOP10 Risiken für Web-Anwendungen gemäß OWASP vertraut machen (siehe xref:glossary:literaturextern:inhalt.adoc#litextern-owasp10[OWASP Top 10])
...
Für detaillierte Informationen über die verwendeten Technologien sei auf entsprechende Literatur verwiesen: xref:glossary:literaturextern:inhalt.adoc#litextern-jsf-wiki[Jakarta Server Faces], xref:glossary:literaturextern:inhalt.adoc#litextern-swf[Spring Web Flow] und xref:glossary:literaturextern:inhalt.adoc#litextern-spring[Spring].
...
Die IsyFact setzt die Referenzimplementierung von Oracle (xref:glossary:literaturextern:inhalt.adoc#litextern-javaserver-faces[JavaServer Faces]) ein und erweitert diese im Bereich der Komponenten um die Bibliothek xref:glossary:literaturextern:inhalt.adoc#litextern-tomahawk[Apache Tomahawk] Apache Tomahawk.
...
Als Implementierung kommt wiederum die Referenzimplementierung von Oracle (xref:glossary:literaturextern:inhalt.adoc#litextern-litextern-javaserver-faces[JavaServer Faces]) zum Einsatz.

[[bearbeitungsmodell-einer-jsf-anfrage]]
==== Bearbeitungsmodell einer JSF Anfrage

Die Spezifikation der Java Server Faces definiert einen sogenannten Lebenszyklus xref:glossary:literaturextern:inhalt.adoc#litextern-jsf-wiki[Jakarta Server Faces] , den eine
...
Durch diese ist es möglich, bereits einen zum xref:isyfact-standards-doku:isy-style:styleguide.adoc[Styleguide] konformen Rahmen zur Verfügung zu stellen, in welchen die
...
Siehe auch Kapitel "Benötigte Bibliotheken" in xref:nutzungsvorgaben/nutzungsvorgaben.adoc[Nutzungsvorgaben JSF].
...
Ein Beispiel dafür ist die GUI einer xref:glossary:glossary:master.adoc#glossar-geschaeftsanwendung[Geschäftsanwendug] zur Datenerfassung, wobei die Speicherung der Daten über einen Service einer anderen Fachanwendung implementiert ist.
...
Für die Arbeit mit JSF werden gemäß dem xref:isyfact-standards-doku:isy-style:styleguide.adoc[Styleguide] sowohl Seitenelemente als auch Bedienelemente bereitgestellt.
...
Die Nutzung der durch den Styleguide definierten Elemente und durch den Baustein bereitgestellten JSF-Komponenten ist ein zentrales Thema der xref:nutzungsvorgaben/nutzungsvorgaben.adoc[Nutzungsvorgaben JSF].
...
.Integration des Bausteins Sicherheit in Spring Security
[id="image-integration-baustein-sicherheit-klassendiagramm",reftext="{figure-caption} {counter:figures}"]
image::konzept/integration-baustein-sicherheit-klassendiagramm.png[align="center"]
...
.Sequenzdiagramm Zugriff auf Sicherheitskomponente
[id="image-integration-baustein-sicherheit-sequenzdiagramm",reftext="{figure-caption} {counter:figures}"]
image::konzept/integration-baustein-sicherheit-sequenzdiagramm.png[align="center"]

Mehr Details hierzu sind in der Bausteindokumentation (xref:isyfact-standards-doku:isy-sicherheit:konzept/master.adoc[Konzept Sicherheit] und xref:isyfact-standards-doku:isy-sicherheit:nutzungsvorgaben/master.adoc[Nutzungsvorgaben Sicherheit]) sowie in den xref:nutzungsvorgaben/nutzungsvorgaben.adoc[Nutzungsvorgaben JSF] zu finden.
...
In diesem Falle wird der Validierungsmechanismus von Spring Web Flow verwendet (s. Abschnitt 5.10 in der Dokumentation des xref:glossary:literaturextern:inhalt.adoc#litextern-jsf-wiki[Jakarta Server Faces], xref:glossary:literaturextern:inhalt.adoc#litextern-swf[Spring Web Flows] ).
