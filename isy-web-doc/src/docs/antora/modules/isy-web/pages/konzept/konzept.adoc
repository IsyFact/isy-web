...
Die Berechtigungsprüfung verwendet dabei den in <<KonzeptSecurity>> beschriebenen Baustein Security der IsyFact.
...
In der Implementierung wird mittels einer Abfrage an `Berechtigungsmanager` aus dem Security-Baustein geprüft, ob ein Anwender die notwendigen Berechtigungen besitzt (siehe dazu <<NutzungsvorgabenSecurity>>).
