...
|====
:linkaktuell: xref:isy-angular-widgets-doc::konzept/konzept.adoc[Baustein isy-web-angular - Konzept]
...
Dieses Konzept baut auf dem xref:3.2.1@isyfact-standards-doku:blaupausen:detailkonzept-komponente-web-gui/master.adoc[Detailkonzept Web-GUI] auf und beschreibt die Architektur der GUI-Komponente, umgesetzt mit den Technologien JSF und Spring Web Flow.
...
Daher wurde diese Option in der xref:3.2.1@isyfact-standards-doku:blaupausen:referenzarchitektur/master.adoc[Referenzarchitektur] ausgeschlossen.
...
Die Architektur von GUIs mit JSF und Spring Web Flow hält sich an die Vorgaben aus der xref:3.2.1@isyfact-standards-doku:blaupausen:referenzarchitektur/master.adoc[Referenzarchitektur] sowie die architektonischen Vorgaben aus dem xref:3.2.1@isyfact-standards-doku:blaupausen:detailkonzept-komponente-web-gui/master.adoc[Detailkonzept Web-GUI].
...
Wie im xref:3.2.1@isyfact-standards-doku:blaupausen:detailkonzept-komponente-web-gui/master.adoc[Detailkonzept Web-GUI] beschrieben, besteht die GUI-Komponente aus Dialog-Komponenten und einem gemeinsamen AWK-Wrapper.
...
Die Berechtigungsprüfung verwendet dabei den xref:3.2.1@isyfact-standards-doku:isy-security:konzept/master.adoc[Baustein Security].
...
In der Implementierung wird mittels einer Abfrage an `Berechtigungsmanager` aus dem Security-Baustein geprüft, ob ein Anwender die notwendigen Berechtigungen besitzt (siehe dazu xref:3.2.1@isyfact-standards-doku:isy-security:nutzungsvorgaben/master.adoc[]).
