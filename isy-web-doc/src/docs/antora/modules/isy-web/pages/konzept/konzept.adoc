...
* Entwickler müssen sich mit den TOP10 Risiken für Web-Anwendungen gemäß OWASP vertraut machen (siehe xref:glossary:literaturextern:inhalt.adoc#litextern-owasp[OWASP Top 10])
...
Für detaillierte Informationen über die verwendeten Technologien sei auf entsprechende Literatur verwiesen: xref:glossary:literaturextern:inhalt.adoc#litextern-jsf-wiki[JavaServer Faces], xref:glossary:literaturextern:inhalt.adoc#litextern-swf[Spring Web Flow] und xref:glossary:literaturextern:inhalt.adoc#litextern-spring[Spring].
...
Die IsyFact setzt die Referenzimplementierung von Oracle (xref:glossary:literaturextern:inhalt.adoc#litextern-javaserver-faces[JavaServer Faces]) ein und erweitert diese im Bereich der Komponenten um die Bibliothek xref:glossary:literaturextern:inhalt.adoc#litextern-tomahawk[Apache Tomahawk].
...
Als Implementierung kommt wiederum die Referenzimplementierung von Oracle (xref:glossary:literaturextern:inhalt.adoc#litextern-javaserver-faces[JavaServer Faces]) zum Einsatz.

[[bearbeitungsmodell-einer-jsf-anfrage]]
==== Bearbeitungsmodell einer JSF Anfrage

Die Spezifikation der Java Server Faces definiert einen sogenannten Lebenszyklus xref:glossary:literaturextern:inhalt.adoc#litextern-jsf-wiki[JavaServer Faces], den eine JSF-Anwendung mit jedem Aufruf erneut durchläuft.
...
Durch diese ist es möglich, bereits einen zum xref:isyfact-standards-doku:isy-styleguide:styleguide.adoc[Styleguide] konformen Rahmen zur Verfügung zu stellen, in welchen die
...
Für die Arbeit mit JSF werden gemäß dem xref:isyfact-standards-doku:isy-styleguide:styleguide.adoc[Styleguide] sowohl Seitenelemente als auch Bedienelemente bereitgestellt.
...
In diesem Falle wird der Validierungsmechanismus von Spring Web Flow verwendet (s. Abschnitt 5.10 in der Dokumentation des xref:glossary:literaturextern:inhalt.adoc#litextern-jsf-wiki[JavaServer Faces], xref:glossary:literaturextern:inhalt.adoc#litextern-swf[Spring Web Flows] ).
